...
class TOFAcqDigitizer():
    ...
    def __init__(self,f):
        ...
        self.ltofB=5000 # %Record length per shot in ns
...
#        
    ...
    def UpdateStream(self,Navg):
        
        success=ADQAPI.ADQ_MultiRecordSetup(self.adq_cu,1,np.int(Navg),self.nsamplesA)
        if (success == 0):
            self.f.write('Recording: ADQ_MultiRecordSetup failed\n')
        else:
            self.f.write('Recording: ADQ_MultiRecordSetup SUCCESS\n')  
        ...
        #success=ADQAPI.ADQ_SetTriggerHoldOffSamples(np.int(self.holdoff_ns*2))
        #self.f.write('DParam: ADQ_SetTriggerHoldOffSamples failed.\n')
