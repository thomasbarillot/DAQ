...
## Multithread workers definition 
class StreamWorker(QtCore.QThread):
    
    def __init__(self,TAQDinstance):
        QtCore.QThread.__init__(self)
        self.myTAQD=TAQDinstance
        self.exiting = False
...
# Start/Stop
         ...
         ## Instanciate the data acquisition object.
         self.myTAQD=TAQD(f)
         
        
         ## Instanciate the Acquisition and Stream Threads
         self.StreamThread=StreamWorker(self.myTAQD)
         self.AcqThread=AcqWorker(self.myTAQD)
         
         self.connect(self.StreamThread, SIGNAL("finished()"), )
         self.connect(self.StreamThread, SIGNAL("terminated()"),)
         self.connect(self.StreamThread, SIGNAL("output(QRect, QImage)"),self.UpdatePlot())
         
         self.connect(self.AcqThread, SIGNAL("finished()"), )
         self.connect(self.AcqThread, SIGNAL("terminated()"),)
         self.connect(self.AcqThread, SIGNAL("output(QRect, QImage)"),)
