...
/**
 * @mainpage scTDC library interface
 * @version 1.6
 *
 * This is documentation for scTDC/scDLD library API. scTDC/scDLD libraries
 * is used for access to Surface Concept GmbH Time to Digital Converter and
 * Delay Line Detector devices.
 */
...
/**
  ...
  @deprecated Use sc_pipe_open2() approach instead.
 */
/*--------------------------------------------------------------------------*/
 ...
 */
/*--------------------------------------------------------------------------*/
SCTDCDLL_PUBLIC int sc_tdc_get_next_histo(unsigned int **histo, int blocking)
DEPRECATED;

/*-------------------------------------------------------------------------*/
/**
  @brief    Sets parameters of FLIM scanner
  @param    histo	Address where the function places the pointer.
  @return   int available bytes
  ...
  @deprecated Use sc_tdc_set_flim_scanner2() approach instead.
 */
/*--------------------------------------------------------------------------*/
 ...
 * @return int Error code
 */
SCTDCDLL_PUBLIC int sc_dbg_set_logger(struct sc_Logger *);
...
SCTDCDLL_PUBLIC int
sc_tdc_init_with_config_lines(const struct sc_ConfigLine *);

SCTDCDLL_PUBLIC int sc_tdc_deinit2(const int);

SCTDCDLL_PUBLIC int
sc_tdc_get_format2(const int, struct sc_tdc_format *format);
...
SCTDCDLL_PUBLIC int sc_tdc_start_measure2(const int, const int ms);

SCTDCDLL_PUBLIC int sc_tdc_interrupt2(const int);

SCTDCDLL_PUBLIC int sc_tdc_get_binsize2(const int, double *binsize_ns);

SCTDCDLL_PUBLIC int sc_tdc_get_status2(const int, int *status);

SCTDCDLL_PUBLIC int
sc_tdc_get_statistics2(const int, struct statistics_t *stat);
...
(const int, const enum sc_pipe_type_t type, const void *params);

SCTDCDLL_PUBLIC int sc_pipe_close2(const int, const int pipe_id);

SCTDCDLL_PUBLIC int sc_pipe_read2
(const int, const int pipe_id, void **buffer, unsigned int timeout);
...
unsigned short line_count,
unsigned int line_delay_interval,
unsigned int multiline_count, double *corr_table);
...
(const int, const char *, char *, size_t, const char *);
